...
soil_moisture_sensor_ns = cg.esphome_ns.namespace("ap_soil_moisture_sensor")
...
CONF_ADC_WET_AT_3V="adc_wet_at_3v"
CONF_BATTERY_LEVEL_RAW="battery_level_raw"
CONF_MOISTURE_RAW="moisture_raw"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_ADC_WET_AT_3V, default=148): cv.int_range(
                min=0,
                max=1024
            ),
        }
...
async def to_code(config):
    ...
    if CONF_MOISTURE_RAW in config:
        sens = await sensor.new_sensor(config[CONF_MOISTURE_RAW])
        cg.add(var.set_moisture_raw(sens))
    cg.add(var.set_adc_dry_at_3v(config[CONF_ADC_DRY_AT_3V]))
    cg.add(var.set_adc_wet_at_3v(config[CONF_ADC_WET_AT_3V]))